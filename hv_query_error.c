...
static int query_injected_errors(int fd)
{
	struct nd_hv_query_injected_errors *e;
	...
	hv_pkg.gen.nd_command = ND_HYPERV_CMD_QUERY_INJECTED_ERRORS;
	hv_pkg.gen.nd_fw_size = 0;
	hv_pkg.gen.nd_size_in = 0;
	hv_pkg.gen.nd_size_out = sizeof(hv_pkg.u.injected_error_info);
	...
	e = &hv_pkg.u.injected_error_info;
	...
	if (e->error_injection_enabled != 1) {
		printf("Error Injection is NOT enabled by the platform.\n");
		return 0;
	}

	printf("Injected Errors = 0x%08x\n", e->injected_errors);

	if (e->injected_errors & (1U << 6))
		printf("Injected Unsafe Shutdown Count = 0x%08x\n",
		       e->injected_shutdown_count);
	else
		printf("No Unsafe Shutdown Count was injected, or the count was cleared.\n");
...
int main()
{
	...
	if (fd < 0) {
		perror("Failed to open /dev/nmem0!\n");
		exit(-1);
	}

	rc = query_injected_errors(fd);
